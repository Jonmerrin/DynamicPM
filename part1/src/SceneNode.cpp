...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera, Camera* unhookedCamera){
    if(m_object!=nullptr){
		if (m_parent != nullptr) {
		...
		m_object->Update(projectionMatrix, camera, unhookedCamera, m_worldTransform);
        ...
        m_shader.SetUniform3f("lightPos", 0, 0, 0.5f);
        m_shader.SetUniform1f("ambientIntensity",0.5f);
		// Iterate through all of the children
		for(int i =0; i < m_children.size(); ++i){
			m_children[i]->Update(projectionMatrix, camera, unhookedCamera);
		}
	}
}
